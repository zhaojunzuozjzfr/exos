...
class DateParser(object):
    ...
    def parse(self, strdate):
        '''
        描述：时间解析方法。
        参数：strdate 要分析的时间字符串，比如目标时间时间类型datetime(1992,2,3)可以被解析
            的是下述字符串之一：
            19920203
            19920203
            1992.02.03
        ...
        flags是标志位，表示有效数字，比如199202实际转换了年和月，日
        没有，但是本函数将默认返回1日，但是flags将表示为(True, True, False)，
        ...
        if m:
            ...
            # results = list(map(lambda x: int(x) if x != None else x, results))
            if results[0] < 100:
                if results[0] > 9:
                    results[0] += 1900
                else:
